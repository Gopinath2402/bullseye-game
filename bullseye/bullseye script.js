...
function loose() {
	...
	document.getElementById('release').volume=0.4;
	count++;
	// console.log(count);
	if (count <=9 || count == 0) {
		document.querySelector('.point').innerHTML="0"+count;
...
function hitTest(tween) {
	...
	if (intersection.segment1 && intersection.segment2) {
		...
		if (distance < 7) {
			...
			if (newCount <=9 ||newCount == 0) {
				document.querySelector('.point2').innerHTML="0" + newCount;
				
			  }
			  if (newCount>9) {
				  document.querySelector('.point2').innerHTML=newCount;
			  }
			document.getElementById('headshot').play();
			document.getElementById('headshot').volume=0.2;
